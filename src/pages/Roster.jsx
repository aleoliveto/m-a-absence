    ...
    <div className="space-y-6">
      ...
      <Card title="Create shift">
        <form onSubmit={createShift} className="grid md:grid-cols-7 gap-3">
          ...
          <Field label="Base"><Input value={newShift.base} onChange={e=>setNewShift(s=>({...s, base:e.target.value}))}/></Field>
        ...
        return (
          <Card key={d} title={new Date(d).toLocaleDateString(undefined,{ weekday:'long', year:'numeric', month:'short', day:'numeric' })}>
            {dayRows.length === 0 ? (
              ...
              <Table head={["Time","Base/Dept","Role","Coverage","Status","Assignments","Actions"]}>
                {dayRows.map(s=>{
